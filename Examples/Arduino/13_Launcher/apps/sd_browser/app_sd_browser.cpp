#include "app_sd_browser.h"

static void sdb_back_cb(lv_event_t *e)
{
  (void)e;
  launcher_request_return_home();
}

lv_obj_t *app_sd_browser_create(void)
{
  ...
  lv_label_set_text(title, "SD 浏览器");
  ...
  lv_obj_add_event_cb(btn, sdb_back_cb, LV_EVENT_RELEASED, NULL);
...
void app_sd_browser_destroy(lv_obj_t *scr)
{
  (void)scr;
}
