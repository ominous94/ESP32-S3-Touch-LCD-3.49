...
SKETCH_PATH = ROOT / "Examples" / "Arduino" / "12_Codex_Status"
BUILD_PATH = ROOT / ".arduino-build" / "codex-status-16m-lvgl9-cdc-opi"
COMPILE_SCRIPT = ROOT / "tools" / "compile_codex_status.ps1"
LOGS_DIR = ROOT / "logs"
FQBN = "esp32:esp32:esp32s3:FlashSize=16M,PartitionScheme=app3M_fat9M_16MB,CDCOnBoot=cdc,PSRAM=opi"
DEFAULT_REQUIRED_TOKENS = ["CODEX_STATUS ui_update", "CODEX_STATUS http_"]
...
def default_log_path():
    ...
    return LOGS_DIR / f"codex_status_serial_{stamp}.log"
...
def evaluate_serial_log(log_path, required_tokens):
    text = Path(log_path).read_text(encoding="utf-8", errors="replace")
    missing = [token for token in required_tokens if token not in text]
    return SerialLogResult(ok=not missing, missing_tokens=missing, log_path=Path(log_path))


def parse_args(argv):
    parser = argparse.ArgumentParser(
        description="Compile, optionally upload, and verify Codex Status serial telemetry."
    )
    ...
    parser.add_argument("--monitor-seconds", default=30, type=int)
...
def main(argv=None):
    ...
    result = evaluate_serial_log(log_path, required_tokens)
    if not result.ok:
